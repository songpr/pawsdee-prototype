...
let TENANT = 'Bangkok Pet Clinic';
// C1 fix — clinic identity comes from tenant config; surfaced on Stage 1 + Stage 3
// This object doubles as the STANDALONE-PREVIEW MOCK (ADR-011): production overwrites its
// fields in place via loadClinicConfig() below once window.PawsDeeData.loadConfig resolves.
const CLINIC = {
  name: TENANT,
...
// ─── Data seam (ADR-011) ─────────────────────────────────────────────────────
// The design calls window.PawsDeeData.* with a built-in mock fallback, so the standalone
// GitHub-Pages preview still plays with no backend. Production injects the real
// implementation via the engineering-owned, un-synced public/journey-data.js (loaded after
// this file — see index.html). journey-data.js contains the only real fetch()/PII-payload
// code; this file and its mocks never do.
async function mockLoadConfig() { return { ...CLINIC }; }
async function mockLoadSlots(slug, date) {
  const seed = parseInt((date || '').replace(/-/g, '').slice(-4), 10) || 0;
  const closed = date ? CLINIC.closedDays.includes(new Date(date + 'T00:00:00').getDay()) : false;
  const slots = filterPastSlots(closed ? [] : buildSlots(seed), date);
  const open = !closed && slots.some((s) => s.available);
  const nearestOpenDate = (closed || (slots.length > 0 && !open)) ? nextOpenISO(date) : undefined;
  return { date, open, slots, nearestOpenDate };
}
async function mockCreateAppointment(slug, payload) {
  await new Promise((r) => setTimeout(r, 700));
  return {
    appointmentId: `mock-${Date.now()}`,
    confirmation: { petName: payload.pet.name, date: payload.date, time: payload.time, clinic: CLINIC.name },
  };
}
async function mockLinkLine() {
  await new Promise((r) => setTimeout(r, 900));
  return { linked: true, cardPushed: true };
}
// Not one of ADR-011's original 4 edges, but the design already ships a "Cancel booking"
// confirm-dialog (SuccessScreen) that must actually release the slot server-side rather
// than just resetting local state (code review 2026-07-03, finding #1).
async function mockCancelAppointment() {
  await new Promise((r) => setTimeout(r, 500));
  return { status: 'cancelled', slotReleased: true };
}
const PD = {
  loadConfig: (...args) => (window.PawsDeeData?.loadConfig ?? mockLoadConfig)(...args),
  loadSlots: (...args) => (window.PawsDeeData?.loadSlots ?? mockLoadSlots)(...args),
  createAppointment: (...args) => (window.PawsDeeData?.createAppointment ?? mockCreateAppointment)(...args),
  linkLine: (...args) => (window.PawsDeeData?.linkLine ?? mockLinkLine)(...args),
  cancelAppointment: (...args) => (window.PawsDeeData?.cancelAppointment ?? mockCancelAppointment)(...args),
  // Engineering hand-edit (2026-07-03, ADR-011 data-seam edge #6 — real LINE linking needs a
  // login redirect in LINE's in-app browser, which drops all in-memory SPA state; this is
  // how PlayApp below resumes straight to the link screen instead of restarting at arrival).
  resumeRelink: (...args) => (window.PawsDeeData?.resumeRelink ?? (() => null))(...args),
};
async function loadClinicConfig(slug) {
  const data = await PD.loadConfig(slug);
  Object.assign(CLINIC, data);
  TENANT = CLINIC.name;
}
// Locale for date rendering — 'auto' follows the browser; the storyboard's header
// toggle persists a forced ไทย/EN as pd-locale, which this playable honours on load.
...
// Chrome/Edge detection (for calendar routing — prefer Google Calendar on those browsers).
const _isChromeOrEdgeBrowser = () => /CriOS|EdgiOS|Chrome/.test(navigator.userAgent);
...
function calPlatform() {
  ...
  if (sim === 'ios')     return { isApple:true,  isIOS:true,  isIOSSafari:true,  inWKWebView:false, isAndroid:false, simulated:'ios', chromeOrEdge:false };
  if (sim === 'line')    return { isApple:true,  isIOS:true,  isIOSSafari:false, inWKWebView:true,  isAndroid:false, simulated:'line', chromeOrEdge:false };
  if (sim === 'android') return { isApple:false, isIOS:false, isIOSSafari:false, inWKWebView:false, isAndroid:true,  simulated:'android', chromeOrEdge:_isChromeOrEdgeBrowser() };
  if (sim === 'desktop') return { isApple:false, isIOS:false, isIOSSafari:false, inWKWebView:false, isAndroid:false, simulated:'desktop', chromeOrEdge:_isChromeOrEdgeBrowser() };
  return { isApple, isIOS, isIOSSafari, inWKWebView, isAndroid: /Android/i.test(ua), chromeOrEdge:_isChromeOrEdgeBrowser() };
}
...
function addToCalendar(form, opts = {}) {
  ...
  // When simulating in the prototype, open targets in a NEW tab + explain — so the
  // running prototype is never navigated away. Real devices navigate in place.
  const sim = plat.simulated;
  const go = (url) => { if (sim) window.open(url, '_blank', 'noopener'); else window.location.href = url; };

  if (plat.isIOSSafari) {
    // Safari native: .ics blob → Calendar.app intercepts natively (1 tap).
    const url = icsBlobURL(form);
    if (sim) calToast('iOS Safari · เปิด .ics → Calendar.app เปิดเลย (1 ขั้นตอน)');
    go(url);                                     // Calendar.app intercepts text/calendar
    setTimeout(() => URL.revokeObjectURL(url), 8000);
    return;
  }
  // Chrome/Edge on any platform: prefer Google Calendar deep link (#6 — user expectation).
  if (plat.chromeOrEdge) {
    const gcalUrl = buildGCalURL(form);
    if (sim) calToast('Chrome/Edge · เปิด Google Calendar');
    go(gcalUrl);
    return;
  }
  if (plat.inWKWebView) {                          // LINE in-app browser — blob is blocked, bounce to Safari
    if (sim) calToast('LINE · เปิด add.html ใน Safari ผ่าน openExternalBrowser=1');
  ...
  if (plat.isAndroid) {
    ...
    let handled = false;
    const onHide = () => { handled = true; };      // app opened → page hidden
    document.addEventListener('visibilitychange', onHide, { once: true });
    window.location.href = deep;                   // 1) try Google Calendar app
    setTimeout(() => {                             // 2) still here? web fallback
      document.removeEventListener('visibilitychange', onHide);
      if (handled || document.hidden) return;
      try { window.location.href = web; }
      catch (e) { window.location.href = landerURL(form); }  // 3) last resort
    }, 1200);
    return;
  }
  openCalSheet(form);                              // desktop / unknown → fallback UI
}
...
// ─────────────────────────────────────────────────────────────
function BookingScreen({ slug, form, setForm, onSubmit }) {
  const [slots, setSlots] = useState([]);
  const [slotsLoading, setSlotsLoading] = useState(true);
  const [slotsError, setSlotsError] = useState(false);
  const [nearestOpen, setNearestOpen] = useState(null);
  const [errors, setErrors] = useState({});
  const [submitting, setSubmitting] = useState(false);

  // Data-seam edge (ADR-011) — real slots come from PD.loadSlots (window.PawsDeeData.loadSlots
  // in production; mockLoadSlots in standalone preview). Both resolve async, so a designed
  // loading/error state replaces the old synchronous buildSlots() call (FR-017).
  const fetchSlots = useCallback((date) => {
    if (!date) return;
    setSlotsLoading(true);
    setSlotsError(false);
    PD.loadSlots(slug, date).then((res) => {
      setSlots(filterPastSlots(res.slots || [], date));
      setNearestOpen(res.nearestOpenDate || null);
  ...
  useEffect(() => { fetchSlots(form.date); }, [form.date, fetchSlots]);
  ...
  const err  = {
    fontSize:'var(--text-xs)', fontWeight:700, marginTop:5,
  ...
  return (
    ...
    <div className="form-scroll" style={{ padding:'var(--space-5) var(--space-5) 12px', display:'flex', flexDirection:'column', gap:6, flex:1, minHeight:0, overflowY:'auto', overflowX:'hidden', boxSizing:'border-box', WebkitOverflowScrolling:'touch' }}>
      ...
      {/* Pre-fill indicator — shown when localStorage data was loaded */}
      {form._prefilled && (
        <div style={{ display:'flex', alignItems:'center', gap:7, padding:'6px 10px 6px 10px',
          background:'var(--teal-50)', border:'1px solid var(--teal-200)',
          borderRadius:'var(--radius-full)', alignSelf:'flex-start', flexShrink:0 }}>
          <Icon name="history" size={13} style={{ color:'var(--teal-600)', flexShrink:0 }} />
          <span style={{ fontSize:'var(--text-xs)', fontWeight:600, color:'var(--teal-700)', whiteSpace:'nowrap' }}>
            ข้อมูลจากการจองครั้งก่อน · From last visit
          </span>
          <button type="button" aria-label="Clear saved data"
            onClick={() => {
              try { localStorage.removeItem('pawsdee_booker'); } catch(e) {}
              setForm(f => ({ ...f, nickname:'', phone:'', petName:'', species:'', _prefilled:false }));
            }}
            style={{ display:'inline-flex', alignItems:'center', justifyContent:'center',
              width:16, height:16, borderRadius:'var(--radius-full)', border:'none',
              background:'var(--teal-300)', color:'white', cursor:'pointer',
              fontSize:11, fontWeight:700, padding:0, flexShrink:0, lineHeight:1 }}>
            ×
          </button>
        </div>
      )}
        ...
        {slotsLoading
          ...
          ? <div style={{ padding:'12px', border:'1.5px dashed var(--neutral-300)', borderRadius:'var(--radius-md)', background:'var(--neutral-50)', display:'flex', flexDirection:'column', gap:8 }}>
              <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                <Icon name="wifi-off" size={15} style={{color:'var(--neutral-400)', flexShrink:0}}/>
                <span style={{fontSize:13, color:'var(--neutral-600)', fontWeight:600}}>ไม่สามารถโหลดช่วงเวลาได้ · Couldn't load timeslots</span>
              </div>
              <button type="button" onClick={() => fetchSlots(form.date)} style={{ alignSelf:'flex-start', display:'inline-flex', alignItems:'center', gap:6, minHeight:40, padding:'8px 14px', fontSize:13, fontWeight:600, fontFamily:'var(--font-en)', color:'white', background:'var(--teal-600)', border:'none', borderRadius:'var(--radius-md)', cursor:'pointer' }}>
                <Icon name="refresh" size={14}/> ลองใหม่ · Retry
              </button>
            </div>
          : slots.length === 0
...
// ─────────────────────────────────────────────────────────────
function ReviewScreen({ slug, form, onConfirm, onBack }) {
  const [confirming, setConfirming] = useState(false);
  const [submitError, setSubmitError] = useState(null);
  // Idempotency key — a ref so it survives re-renders AND retries after a failed submit
  // (same key on retry ⇒ the backend returns the already-created row, never a duplicate).
  const idemKey = useRef(`${slug}-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  // Data-seam edge (ADR-011): the real POST happens in PD.createAppointment
  // (window.PawsDeeData.createAppointment in production; mockCreateAppointment in preview).
  const confirm = (btnRef) => {
    // Instant visual disable — pointer-events:none bypasses React's render cycle latency.
    if (btnRef) btnRef.style.pointerEvents = 'none';
    setConfirming(true);
    setSubmitError(null);
    PD.createAppointment(slug, {
      idempotencyKey: idemKey.current,
      date: form.date,
      time: form.time,
      owner: { name: form.nickname, phone: form.phone },
      pet: { name: form.petName, species: form.species },
      reason: form.reason,
    }).then((res) => {
      setConfirming(false);
      onConfirm(res.appointmentId, res.alreadyLinked);
    }).catch((e) => {
      // Restore button on error so user can retry (same idemKey = backend returns existing row).
      if (btnRef) btnRef.style.pointerEvents = '';
      setConfirming(false);
      setSubmitError((e && e.code) || 'failed');
    });
  };
  ...
  return (
    ...
    <div className="screen">
      ...
      {submitError && (
        <div className="alert alert-warning">
          <Icon name="alert-circle" style={{ fontSize:18, marginTop:1, flexShrink:0 }} />
          <div>
            {submitError === 'slot_unavailable'
              ? 'ช่วงเวลานี้เพิ่งถูกจอง กรุณาเลือกใหม่ · This slot was just taken — please pick another.'
              : 'เกิดข้อผิดพลาด กรุณาลองใหม่ · Something went wrong — please try again.'}
          </div>
        </div>
      )}
    </div>
    ...
    <div className="cta-bar">
      <button className="btn btn-primary" disabled={confirming} onClick={(e)=>confirm(e.currentTarget)}>
...
// ─────────────────────────────────────────────────────────────
function SuccessScreen({ slug, form, onLink, onSkip, onCancel = () => {} }) {
  const [confirmCancel, setConfirmCancel] = useState(false);
  const [cancelling, setCancelling] = useState(false);
  const [cancelError, setCancelError] = useState(null);
  // Data-seam edge: the real POST /appointments/:id/cancel happens in PD.cancelAppointment
  // — this dialog previously only reset local state while telling the owner the slot was
  // released, without ever calling the backend (code review 2026-07-03, finding #1).
  const doCancel = () => {
    setCancelling(true);
    setCancelError(null);
    PD.cancelAppointment(slug, form.appointmentId).then(() => {
      setCancelling(false);
  ...
  return (
    ...
    <div style={{ flex:1, overflowY:'auto', WebkitOverflowScrolling:'touch', display:'flex', flexDirection:'column', gap:'var(--space-3)', padding:'var(--space-4) var(--space-4) var(--space-10)' }}>
      ...
      <div className="summary">
        ...
        <div className="summary-row" style={{ alignItems:'flex-start' }}>
          ...
          <a href={CLINIC.mapUrl || `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent((CLINIC.name||'')+','+(CLINIC.locality||''))}`} target="_blank" rel="noopener" style={{ display:'inline-flex', alignItems:'center', gap:5, fontSize:'var(--text-xs)', fontWeight:600, color:'var(--teal-700)', textDecoration:'none', textAlign:'right', maxWidth:170, lineHeight:1.4 }}>
    ...
    <div style={{ flexShrink:0, padding:'12px 16px 18px', background:'white', borderTop:'1px solid var(--neutral-200)', boxShadow:'0 -4px 12px rgba(0,0,0,0.06)', display:'flex', flexDirection:'column', gap:8 }}>
      {form.alreadyLinked ? (
        // Customer is already linked to LINE — skip the ask-to-link prompt (#UX-7). Still
        // needs the same "Open LINE chat" affordance AccountLinkedScreen offers right after a
        // live link — otherwise a repeat customer who's already linked has no way from THIS
        // screen to reach the clinic's OA chat where their reminder card actually lives (found
        // reviewing a real UAT screenshot: the fix skipped the prompt but dropped the chat link
        // entirely, leaving only Add to calendar + Done).
        <>
          <div style={{ textAlign:'center', fontSize:'var(--text-xs)', color:'var(--line-green)', fontWeight:600, display:'flex', alignItems:'center', justifyContent:'center', gap:5 }}>
            <Icon name="circle-check" size={14}/> เชื่อม LINE แล้ว · Already linked to LINE
          </div>
          {CLINIC.lineOAId && (
            <a href={`https://line.me/R/ti/p/${encodeURIComponent(CLINIC.lineOAId)}`} target="_blank" rel="noopener noreferrer"
              className="btn btn-line" style={{ fontWeight:700, display:'flex', alignItems:'center', justifyContent:'center', gap:8, textDecoration:'none', color:'#fff' }}>
          ...
          {/* Done — primary only when there's no LINE OA configured to open instead */}
          <button className={CLINIC.lineOAId ? undefined : "btn btn-line"} type="button" onClick={onSkip} style={CLINIC.lineOAId ? {
            ...
            <Icon name="check" size={CLINIC.lineOAId ? 16 : 20}/> เสร็จสิ้น · Done
          </button>
        </>
      ) : (
        // Not linked yet — show Link LINE prompt
        <>
        ...
        <div onClick={(e)=>e.stopPropagation()} style={{ width:'100%', background:'white',
          ...
          {cancelError && (
            <div className="alert alert-warning" style={{ marginTop:8, textAlign:'left' }}>
              <Icon name="alert-circle" style={{ fontSize:18, marginTop:1, flexShrink:0 }} />
              <div>ยกเลิกไม่สำเร็จ กรุณาลองใหม่ · Couldn't cancel — please try again.</div>
            </div>
          )}
          <div style={{ display:'flex', flexDirection:'column', gap:8, marginTop:'var(--space-4)' }}>
            <button className="btn btn-secondary" disabled={cancelling} style={{ width:'100%', justifyContent:'center' }} onClick={()=>setConfirmCancel(false)}>
              <Icon name="arrow-back-up" size={16}/> เก็บนัดไว้ · Keep booking
            </button>
            <button className="btn btn-danger" disabled={cancelling} style={{ width:'100%', justifyContent:'center' }} onClick={doCancel}>
              {cancelling ? <><Icon name="loader-2" className="spin" size={16}/> กำลังยกเลิก…</> : <><Icon name="calendar-x" size={16}/> ยกเลิกนัด · Cancel booking</>}
            </button>
          </div>
        </div>
      </div>
...
// ─────────────────────────────────────────────────────────────
function LinkingScreen({ slug, appointmentId, nickname, petName, onDone, expired = false }) {
  ...
  // Auto-proceed after LIFF login redirect returns (?relink=1).
  // When LINE OAuth redirects back, LIFF is now logged in — no need to ask the user 
  // to tap "Connect" again. Detect this on mount and POST /link automatically (#4).
  useEffect(() => {
    const cameFromRedirect = typeof window !== 'undefined' &&
      new URLSearchParams(window.location.search).get('relink');
    if (cameFromRedirect && !expired && !linking) {
      // LIFF should be logged in now — try auto-link with a small delay for init to settle.
      const timer = setTimeout(() => {
        if (typeof window !== 'undefined' && window.liff && window.liff.isLoggedIn && window.liff.isLoggedIn()) {
          link();
        } else {
          // LIFF not ready yet — show the screen so user can tap manually as fallback.
          setLinking(false);
        }
      }, 800);
  ...
  // Data-seam edge (ADR-011): the real POST /link happens in PD.linkLine (window.PawsDeeData
  // .linkLine in production; mockLinkLine in preview). lineUserToken comes from the LIFF SDK
  // when present (US3 wires the LIFF init call); absent here it posts empty and the backend
  // — once US3 ships — rejects it, surfacing linkError below rather than a false success.
  const link = () => {
    setLinking(true);
    setLinkError(null);
    PD.linkLine(slug, {
      appointmentId,
      lineUserToken: (typeof window !== 'undefined' && window.liff && window.liff.getAccessToken && window.liff.getAccessToken()) || '',
      consents: { reminders: true, marketing: consent.marketing },
    }).then(() => {
      setLinking(false);
      // Linking succeeds → advance straight to the single linked terminal (Stage 5,
      // AccountLinkedScreen) via onDone(). There is deliberately no separate in-screen
      // "linked" view here — that was a duplicate success page before the confirmed one.
      onDone();
    }).catch(() => {
      setLinking(false);
      setLinkError('link_failed');
    });
  };
    ...
    <div className="screen stack" style={{ gap:'var(--space-3)', paddingBottom:'var(--space-16)', overflowY:'auto', WebkitOverflowScrolling:'touch' }}>
      ...
      {expired ? (
        ...
        {linkError && (
          <div className="alert alert-warning">
            <Icon name="alert-circle" style={{ fontSize:18, marginTop:1, flexShrink:0 }} />
            <div>เชื่อมบัญชีไม่สำเร็จ กรุณาลองใหม่ · Couldn't link your account — please try again.</div>
          </div>
        )}
    ...
    <div className="cta-bar" style={{ display:'flex', flexDirection:'column', gap:8 }}>
      {CLINIC.lineOAId ? (
        ...
        <button className="btn btn-line" onClick={onDone} style={{ fontWeight:700 }}>
          <Icon name="brand-line" size={20}/> เสร็จสิ้น · Done
        </button>
      )}
...
// NOTE: there is deliberately no separate "Finished / All done" screen. The real
// product terminal is the Linked! screen (AccountLinkedScreen); "Done" / "Open LINE
// chat" — and "Skip for now" on the confirmed screen — simply end the flow. In this
// playable demo they reset to Booking so the journey can be replayed (the floating
// Restart pill does the same). Shipped flow: Confirmed → Linking → Linked. Full stop.

const PLAY_INIT = () => {
  try {
    const saved = JSON.parse(localStorage.getItem('pawsdee_booker') || 'null');
    if (saved && (saved.nickname || saved.phone || saved.petName)) {
      return {
        nickname: saved.nickname || '', phone: saved.phone || '',
        petName: saved.petName || '', species: saved.species || '',
        date: futureISO(3), time: '', reason: '', _prefilled: true,
      };
    }
  } catch(e) {}
  return { nickname:'', phone:'', petName:'', species:'', date: futureISO(3), time:'', reason:'' };
};
const PLAY_STAGES = [
  { key:'booking',   num:'1', th:'จองนัด',      en:'Booking' },
  { key:'review',    num:'2', th:'ตรวจสอบ',     en:'Review' },
  { key:'confirmed', num:'3', th:'นัดสำเร็จ',    en:'Confirmed' },
  { key:'link',      num:'4', th:'เชื่อม LINE', en:'Link LINE' },
  { key:'linked',    num:'5', th:'เชื่อมสำเร็จ', en:'Linked' },
];

// ─────────────────────────────────────────────────────────────
// Playable shell — full viewport, no bezel, no canvas chrome.
// Feels like the real app on mobile; centered at 430px on desktop.
// Tiny floating restart pill for prototype testing only.
// ─────────────────────────────────────────────────────────────
function PlayApp() {
  // Slug from the URL (ADR-011 T061): /booking/:slug. Falls back to 'demo' for the
  // standalone preview (served at a bare path with no backend behind it).
  const slug = (typeof location !== 'undefined' && location.pathname.match(/^\/booking\/([^/]+)/)?.[1]) || 'demo';
  const [configReady, setConfigReady] = useState(false);
  const [configError, setConfigError] = useState(false);
  // Monotonically-increasing counter: increments every time stage changes to 'booking',
  // forcing BookingScreen to remount and re-fetch slots (#1 — booked slot still shown available).
  const [slotRefresh, setSlotRefresh] = useState(0);
  // Engineering hand-edit (2026-07-03) — resume straight to the link screen after a real
  // LINE login redirect (PD.linkLine triggers liff.login() when not logged in; that's a
  // full-page navigation away and back, which would otherwise reset this stateless SPA to
  // the arrival screen and lose which appointment the owner was linking).
  const [resumed] = useState(() => PD.resumeRelink());
  const [stage, setStage] = useState(resumed ? 'link' : 'booking');
  const [form, setForm] = useState(() => (resumed ? { ...PLAY_INIT(), appointmentId: resumed.appointmentId } : PLAY_INIT()));

  // Data-seam edge (ADR-011): resolve the real tenant config via PD.loadConfig before the
  // journey renders, so the arrival state never flashes the mock demo clinic's name/address —
  // and an unknown/malformed slug shows a designed error (FR-017/FR-018), never the demo
  // clinic's identity standing in for a clinic that doesn't exist.
  // Increment slotRefresh whenever entering the booking stage — forces fresh slot fetch (#1).
  const prevStageRef = React.useRef(stage);
  if (stage === 'booking' && prevStageRef.current !== 'booking') {
  ...
  useEffect(() => {
    let cancelled = false;
    loadClinicConfig(slug)
      .then(() => { if (!cancelled) setConfigReady(true); })
      .catch(() => { if (!cancelled) { setConfigError(true); setConfigReady(true); } });
    return () => { cancelled = true; };
  }, [slug]);

  const restart = () => { setForm(PLAY_INIT()); setStage('booking'); };
  const saveBooker = () => {
    try {
      localStorage.setItem('pawsdee_booker', JSON.stringify({
  ...
  let screen = null;
  if (stage === 'booking')   screen = <BookingScreen key={`bs-${slotRefresh}`} slug={slug} form={form} setForm={setForm} onSubmit={()=>{ saveBooker(); setStage('review'); }} />;
  if (stage === 'review')    screen = <ReviewScreen slug={slug} form={form} onConfirm={(appointmentId, alreadyLinked)=>{ saveBooker(); setForm((f)=>({ ...f, appointmentId, alreadyLinked })); setStage('confirmed'); }} onBack={()=>setStage('booking')} />;
  if (stage === 'confirmed') screen = <SuccessScreen slug={slug} form={form} onLink={()=>setStage('link')} onSkip={restart} onCancel={restart} />;
  if (stage === 'link')      screen = <LinkingScreen slug={slug} appointmentId={form.appointmentId} nickname={form.nickname} petName={form.petName} onDone={()=>setStage('linked')} />;
  if (stage === 'linked')    screen = <AccountLinkedScreen form={form} onDone={restart} />;

  return (
    <div style={{ height:'100dvh', overflow:'hidden', display:'flex', justifyContent:'center', background:'var(--neutral-200)' }}>
      <div style={{
        position:'relative', width:'100%', maxWidth:430,
        height:'100%', display:'flex', flexDirection:'column',
        background:'var(--neutral-50)',
        boxShadow:'0 0 48px rgba(0,0,0,0.10)',
      }}>
        <AppHeader />
        {!configReady ? (
          <div className="skeleton-grid" style={{ padding:'var(--space-5)', display:'flex', flexDirection:'column', gap:'var(--space-2)' }}>
            <div className="skeleton skeleton-line" />
            <div className="skeleton" />
          </div>
        ) : configError ? (
          <div className="screen" style={{ padding:'var(--space-5)', textAlign:'center' }}>
            <SectionHeader icon="alert-triangle" titleTh="ไม่พบคลินิก" titleEn="Clinic not found"
              descTh="กรุณาตรวจสอบลิงก์อีกครั้ง" descEn="Please check the booking link and try again" />
          </div>
        ) : screen}
      </div>
    </div>
...
// ── THIS FILE IS THE SOURCE OF TRUTH (ADR-012). DO NOT run /design-sync against it. ──────
// As of ADR-012 (2026-07-04) the repo is canonical for the live LIFF journey; Claude Design
// (`c4552d25`) is demoted to an ideation sandbox and `/design-sync` is RETIRED for this
// journey — running it would OVERWRITE this file with the pre-UAT design export and revert
// every fix below. The prototype preview is now DERIVED from this file by `make
// publish-preview`, not the other way round.
//
// The list below is a HISTORICAL record of the engineering-origin edits that diverged this
// file from the original T060 Claude Design export (`journey-play.jsx`). It is kept ONLY so
// that IF a future ADR-012 trigger reintroduces a design-tool workflow, the design source can
// be caught up first (with an explicit reverse-sync/drift story — the lesson of gotchas
// 0037/0040). Nothing here is "pending back-port" for correctness — these ARE the journey now.
//
//   1. PD.resumeRelink added to the PD object (2026-07-03) — resumes straight to the link
//      screen after a real LINE OAuth redirect (page navigates away and back; a stateless SPA
//      would otherwise reset to arrival and lose which appointment was being linked).
//   2. Removed the floating "Restart" pill overlay (2026-07-03) — prototype-testing-only
//      chrome, not appropriate in the production journey.
//   3. calPlatform()'s Chrome/Edge detection (_isChromeOrEdgeBrowser, `chromeOrEdge` field on
//      every branch incl. `sim` overrides) + addToCalendar()'s new chromeOrEdge branch,
//      prioritizing the Google Calendar deep link over the .ics blob on Chrome/Edge (2026-07-03
//      UAT finding #6 — only iOS Safari auto-opens Calendar.app on a .ics blob).
//   4. ReviewScreen's confirm() takes a btnRef and sets pointer-events:none synchronously on
//      click (instant double-tap guard, ahead of React's render cycle) + passes
//      res.alreadyLinked through to onConfirm (2026-07-03/07-04, UAT finding #3 + #7).
//   5. SuccessScreen's map link falls back to a generated Google Maps search URL
//      (name+locality) when CLINIC.mapUrl is empty, instead of a dead href="" (2026-07-03,
//      UAT finding #2).
//   6. SuccessScreen's CTA bar is now a form.alreadyLinked ? … : … branch: an already-linked
//      customer sees "Already linked to LINE" + an "Open LINE chat" link (if CLINIC.lineOAId
//      is configured) + Add to calendar + Done, instead of being asked to link again
//      (2026-07-04, UAT finding #7/#7b).
//   7. LinkingScreen auto-proceeds (calls link() automatically) when arriving via ?relink=1
//      and LIFF is already logged in, instead of requiring a second manual tap of "เชื่อมบัญชี
//      LINE" (2026-07-03, UAT finding #4).
//   8. AccountLinkedScreen's "Open LINE chat" is a real `<a href="https://line.me/R/ti/p/…">`
//      link when CLINIC.lineOAId is configured, instead of a button that just called onDone
//      and did nothing LINE-related (2026-07-03, UAT finding #5).
//   9. PlayApp: slotRefresh counter + a prevStageRef effect force BookingScreen to remount
