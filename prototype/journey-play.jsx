...
function addToCalendar(form, opts = {}) {
  ...
  if (plat.inWKWebView) {                          // LINE / Chrome iOS — blob is blocked
...
function PlayApp() {
  ...
  const [form, setForm] = useState(PLAY_INIT);
  ...
  let screen = null;
  if (stage === 'booking')   screen = <BookingScreen form={form} setForm={setForm} onSubmit={()=>{ saveBooker(); setStage('review'); }} />;
  if (stage === 'review')    screen = <ReviewScreen form={form} onConfirm={()=>{ saveBooker(); setStage('confirmed'); }} onBack={()=>setStage('booking')} />;
  if (stage === 'confirmed') screen = <SuccessScreen form={form} onLink={()=>setStage('link')} onSkip={restart} onCancel={restart} />;
  if (stage === 'link')      screen = <LinkingScreen nickname={form.nickname} petName={form.petName} onDone={()=>setStage('linked')} />;
  ...
  return (
    <div style={{ height:'100dvh', overflow:'hidden', display:'flex', justifyContent:'center', background:'var(--neutral-200)' }}>
      <div style={{
        ...
        <AppHeader />
        {screen}
        {/* Restart overlay — sits on top of the AppHeader content row (below 54px status bar)
            so it adds zero height and real scroll testing is unaffected. */}
        <button onClick={restart} style={{
          position:'absolute', top:14, right:14, zIndex:10,
          display:'inline-flex', alignItems:'center', gap:4,
          background:'rgba(255,255,255,0.85)', backdropFilter:'blur(8px)',
          border:'1px solid var(--neutral-200)', borderRadius:999,
          padding:'4px 10px', fontSize:10, fontWeight:700, color:'var(--neutral-600)',
          cursor:'pointer', fontFamily:'var(--font-en)',
          boxShadow:'0 1px 4px rgba(0,0,0,0.10)',
        }}>
          <Icon name="refresh" size={10}/> Restart
        </button>
      </div>
    </div>
  );
